  ...
  <q-page padding>
    ...
    <q-dialog v-model="prompt" persistent>
      <q-card style="min-width: 350px">
        <q-card-section>
          <div class="text-h6"> {{ labelText }} Insurer</div>
        </q-card-section>

        <q-card-section class="q-pt-none">
          <q-input label="Insurer" :disable="actionType === 'view'" v-model="insurer" stack-label :rules="[ val => val && val.length > 0 || 'Required']"/>
        </q-card-section>
    ...
    <div class="q-pa-md">
      <q-btn @click="openDialog('add', { id: null, insurer: '' })" color="grey-8" icon="add" label="New Insurer"/>
    </div>
    <div class="q-pa-md">
      <q-table
      title="Insurer Module"
      :data="tableData"
      :columns="columns"
      separator="cell"
    >
  ...
  methods: {
    ...
    performAction () {
      ...
      if (this.actionType === 'update') {
        action = this.$axios.patch(`/api/insurer/${this.id}`, { data: { insurer: this.insurer } })
      } else {
        action = this.$axios.post('/api/insurer', { insurer: this.insurer })
      }
      action.then(res => {
        this.$q.loading.hide()
        this.$q.notify({

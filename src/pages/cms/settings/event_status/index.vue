<template>
  <q-page padding>
    ...
    <q-dialog v-model="prompt" persistent>
      <q-card style="min-width: 350px">
        ...
        <q-card-section class="q-pt-none">
          <q-input label="Event Status" :disable="actionType === 'view'" v-model="status" stack-label :rules="[ val => val && val.length > 0 || 'Required']"/>
        </q-card-section>
    ...
    <div class="q-pa-md">
      <q-btn @click="openDialog('add', { id: null, status: '' })" color="grey-8" icon="add" label="New Event Status"/>
    </div>
    <div class="q-pa-md">
      <q-table
      title="Event Status Module"
      :data="tableData"
      :columns="columns"
      separator="cell"
    >
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { align: 'left', name: 'status', label: 'Status Name', field: 'status', sortable: true },
  ...
  methods: {
    ...
    performAction () {
      ...
      if (this.actionType === 'update') {
        action = this.$axios.patch(`/api/status_lists/${this.id}`, { data: { status: this.status } })
      } else {
        action = this.$axios.post('/api/status_lists', { status: this.status })
      }
      action.then(res => {
        this.$q.loading.hide()
        this.$q.notify({
